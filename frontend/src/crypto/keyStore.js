...
function privateKeyId(userId) {
    return userId ? `privateKey:${userId}` : LEGACY_PRIVATE_KEY_ID;
}
function publicKeyId(userId) {
...
// 🔐 Save private key
export async function savePrivateKey(key, userId) {
...
export async function savePublicKey(publicKey, userId) {
    const db = await openDB();
