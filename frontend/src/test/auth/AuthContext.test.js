...
function AuthProbe() {
    const { user } = useAuth();
    return (_jsxs("div", { children: [_jsx("span", { "data-testid": "user-email", children: user?.email ?? "anon" }), _jsx("span", { "data-testid": "user-id", children: user?.id ?? -1 }), _jsx("span", { "data-testid": "path", children: window.location.pathname }), _jsx("span", { "data-testid": "search", children: window.location.search })] }));
}
...
const renderProvider = (initialUrl) => {
    // jsdom's location is read-only via assignment; replaceState works.
    window.history.replaceState({}, "", initialUrl);
    return render(_jsx(MemoryRouter, { children: _jsx(AuthProvider, { children: _jsx(AuthProbe, {}) }) }));
};
